...
def parse_args()
    ...
    parser.add_argument('--pretrained_controlnet', type=str, required=True)
...
def calculate_loss_weights(image_size, bounding_boxes, device, min_weight, max_weight, global_step, total_steps, pct_start=0.3)
    ...
    for idx, box_set in enumerate(bounding_boxes)
        for box in box_set
            ...
            weights[idx, 0, y1y2, x1x2] = current_weight
    return weights
...
def train(rank, world_size, num_epochs=100)
    ...
    device = torch.device(fcuda{rank})

    tokenizer = CLIPTokenizer.from_pretrained(args.sd_path, subfolder='tokenizer')
    tokenizer_2 = CLIPTokenizer.from_pretrained(args.sd_path, subfolder='tokenizer_2')
    text_encoder = CLIPTextModel.from_pretrained(args.sd_path, subfolder='text_encoder').to(device)
    text_encoder_2 = CLIPTextModelWithProjection.from_pretrained(args.sd_path, subfolder='text_encoder_2').to(device)
    ...
    train_loader = DataLoader(dataset, batch_size=1, collate_fn=collate_fn, sampler=train_sampler, num_workers=4, pin_memory=True)
    ...
    for epoch in range(num_epochs)
        ...
        for i, data in enumerate(train_loader)
            ...
            with torch.no_grad()
                text_input_ids1 = tokenizer(data['prompts'], padding=max_length, max_length=tokenizer.model_max_length, truncation=True, return_tensors=pt).input_ids.to(device)
                text_input_ids2 = tokenizer_2(data['prompts'], padding=max_length, max_length=tokenizer_2.model_max_length, truncation=True, return_tensors=pt).input_ids.to(device)

                text_outputs1 = text_encoder(text_input_ids1, output_hidden_states=True)
                text_outputs2 = text_encoder_2(text_input_ids2, output_hidden_states=True)

                prompt_embeds = torch.cat([text_outputs1.hidden_states[-2], text_outputs2.hidden_states[-2]], dim=-1)
                pooled_prompt_embeds2 = text_outputs2[0]

                add_time_ids = torch.tensor([(1024, 1024, 0, 0, 1024, 1024)], dtype=torch.float32).to(device).repeat(len(data['prompts']), 1)
            ...
            noisy_latents = DDIMScheduler.from_pretrained(args.sd_path, subfolder=scheduler).add_noise(latents, noise, timesteps)
            ...
            if (i + 1) % accumulation_steps == 0
                ...
                if rank == 0
                    writer.add_scalar('Losstrain', weighted_loss.item(), global_step)
        ...
        if rank == 0
            print(fEpoch {epoch+1}, Loss {epoch_loss}, flush=True)
